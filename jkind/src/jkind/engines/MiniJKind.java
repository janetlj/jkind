...
public class MiniJKind extends Engine {
	...
	public MiniJKind(Specification spec, JKindSettings settings) {
		...
		if (settings.allAssigned && settings.reduceIvc){
			...
			this.director = new Director(settings, new Specification(new Program(newNode), settings.slicing),
					new Specification(new Program(newNode), settings.slicing), this);
		}else if (!settings.allAssigned && settings.reduceIvc){
			this.director = new Director(settings, new Specification(new Program(spec.node), settings.slicing),
					new Specification(new Program(spec.node), settings.slicing), this);
		}else{
